...
| `f_video_chat_rag.ipynb` | 화상채팅 요약 RAG 프로젝트 | 질문 단위 시맨틱 캐시, metadata filter, 비동기 LCEL |
...
## 7. 화상채팅 요약 RAG 프로젝트 (`f_video_chat_rag.ipynb`)

앞에서 학습한 RAG, LCEL, 시맨틱 캐시 개념을 종합해 실제 서비스 형태로 적용한 프로젝트이다.
DB에 저장된 화상채팅 요약을 검색 대상으로 삼아, 로그인한 회원이 자신의 회의 내용을 질의응답할 수 있게 한다.

### 핵심 설계 포인트

| 포인트 | 내용 |
| :--- | :--- |
| 질문 단위 시맨틱 캐시 | RAG 최종 프롬프트에 전역 `RedisSemanticCache`를 걸면, 긴 공통 프롬프트/Context 때문에 서로 다른 질문도 같은 답변으로 캐시 히트될 수 있다. 그래서 전역 LLM 캐시는 끄고, `member_id + video_session_id + question`을 임베딩한 답변 캐시를 RAG 앞단에 둔다 |
| metadata filter | 기본 검색기는 유지하되, `video_session_id`가 들어오면 metadata 필터로 해당 화상회의 요약만 검색해 다른 회의 내용 혼입을 줄인다 |
| 한국어 임베딩 | 한국어 검증 모델 `jhgan/ko-sbert-nli`를 로컬 CPU에서 사용한다 |
| Context 제한 프롬프트 | Context 안의 정보만 사용하고, 자료에 없으면 "확인이 불가능하다"고 답하게 한다 |
| 비동기 호환 LCEL | LCEL 체인은 `invoke`/`ainvoke`를 모두 지원한다. 정의는 동일하고 호출 시점에만 비동기 여부가 결정되어, FastAPI 엔드포인트에서 `await chain.ainvoke(...)`로 그대로 재사용할 수 있다 |

### 파이프라인 단계

| 단계 | 내용 | 기반 개념 |
| :--- | :--- | :--- |
| 시맨틱 캐시 설정 | 질문 단위 답변 캐시 (전역 LLM 캐시는 끔) | `a_LLM.ipynb`의 Redis 벡터 검색 흐름 |
| 문서 분할 | 회의 1건 요약이 짧으면 회의 1건 = 청크 1개 | `e_RAG.ipynb` |
| 임베딩 | `jhgan/ko-sbert-nli`, 로컬 CPU | `e_RAG.ipynb` |
| 벡터 DB | FAISS | `e_RAG.ipynb` |
| 검색기 | 기본 검색기 + `video_session_id` metadata 필터 | `e_RAG.ipynb` |
| 프롬프트 | Context 내 정보만 사용 | `e_RAG.ipynb` |
| LLM | 프로젝트 기준 모델 사용 | `e_RAG.ipynb` |
| 체인 | LCEL 비동기 호환 (`invoke`/`ainvoke`) | `e_RAG.ipynb` + `c_LCEL.ipynb` 비동기 패턴 |
...
4. `video_session_id` metadata 필터가 다른 회의 요약 혼입을 줄인다.
